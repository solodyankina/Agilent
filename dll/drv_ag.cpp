...
extern "C" __declspec(dllexport)__stdcall unsigned int Drv_StartTestWithMask (int flen, char*filename);
extern "C" __declspec(dllexport)__stdcall unsigned int Drv_SetSignalMask (double dx, double dy);
extern "C" __declspec(dllexport)__stdcall unsigned int Drv_StartTest(int flen, char * filename);
...
extern "C" __declspec(dllexport)__stdcall unsigned int Drv_SaveWaveData (char*filename,char*fmt);
...
unsigned int __stdcall Drv_TriggerDelay(unsigned int  param, unsigned int param2, double param3,double param4)
{
	...
	if (err != Error_NoError)
			return err;
	str =  "The :TRIGger:DELay:TRIGger:COUNt " + MyFloatToStr(param4) + "\n" ;
	err = viPrintf(vi, str.c_str());
	if (err != Error_NoError)
			return err;
	return Error_NoError;
...
unsigned int __stdcall Drv_TriggerGlitch(unsigned int param, unsigned int param2, double param3,double param4)
{
	...
	if (param2 == Agilent_greaterthan)
	{
		...
		if (err != Error_NoError)
			return err;
		str = ":TRIGger:GLITch:GREaterthan " + MyFloatToStr(param3) + "us\n";
		err = viPrintf(vi, str.c_str());
		if (err != Error_NoError)
			return err;
	}
	if (param2 == Agilent_lessthan)
	{
		...
		if (err != Error_NoError)
			return err;
		str = ":TRIGger:GLITch:LESSthan " + MyFloatToStr(param4) + "us\n";
		err = viPrintf(vi, str.c_str());
		if (err != Error_NoError)
			return err;
	}
...
unsigned int __stdcall Drv_StartTestWithMask(int flen, char* filename)
{
double val;
	ofstream f;
	bStopAll = false;
	...
	unsigned int size;
	AnsiString file_name = ExtractFilePath(filename);
	int k=0,j=0;
	dx = 20;
	dy = 20;
		...
		if ((str == "+32\n") || (str == "+544\n"))
		{
			...
			do
			{
				...
				for (int i = 0; i < size; i++)
				{
					...
					if (dt > 0 )
					{
						mydata[i] = -(fabsl(dt - yreference)*yincrement)+yorigin;
					}
					else
					{
						mydata[i] = (fabsl(-dt - yreference)*yincrement)+yorigin;
...
unsigned int __stdcall Drv_StartTest(int flen, char * filename)
{
	double val;
	ofstream f;
	bStopAll = false;
		...
		if ((str == "+32\n") || (str == "+544\n"))
		{
			...
			do
			{
					...
					||(Check_Range(maxOstU,0,maxOU)==0)||(Check_Range(minOstU,0,maxOU)==0))
				{
					...
					if ((Check_Range(maxAmplit,minA,maxA)==0)||(Check_Range(minAmplit,minA,maxA)))
					{
						f<< "<tr><td> Амплитуда (В)</td><td> "<<minAmplit<<" </td><td> "<<maxAmplit<<" </td><td>"<<minA<<".."<<maxA<<"B</td></tr> ";
					}
					if ((Check_Range(maxOvershoot,minOv,maxOv)==0)||(Check_Range(minOvershoot,minOv,maxOv)==0)) {
						f<< "<tr><td> Колебания (В) </td><td> "<<minOvershoot<<" </td><td>"<<maxOvershoot<<"</td><td> 0..0.3 B</td></tr>";
					}
					if ((Check_Range(maxRis,minR,maxR)==0)||(Check_Range(minRis,minR,maxR)==0)) {
						f<< "<tr><td> Нарастание фронта (нс) </td><td> "<<minRis*1000000000<<" </td><td>"<<maxRis*1000000000<<"</td><td>"<<minR*1000000000<<".."<<maxR*1000000000<<" нс </td></tr>";
					}
					if ((Check_Range(maxFall,minF,maxF)==0)||(Check_Range(minFall,minF,maxF)==0)) {
						f<< "<tr><td> Спад фронта (нс) </td><td> "<<minFall*1000000000<<" </td><td>"<<maxFall*1000000000<<"</td><td>"<<minF*1000000000<<".."<<maxF*1000000000<<" </td></tr>";
					}
					if (bFindWaveEnd == true){

						if ((Check_Range(maxOstU,0,maxOU)==0)||(Check_Range(minOstU,0,maxOU)==0)) {
							f<< "<tr><td> Остаточное напряжение (В) </td><td> "<<minOstU<<" </td><td>"<<maxOstU<<"</td><td>"<<minOU<<".."<<maxOU<<" B</td></tr>";
						}
					}
					f<<"</table>";
...
unsigned int __stdcall Drv_StopTest()
{
	bStopAll = true;
	return Error_NoError;
}
...
{
	str = ":CHANnel1:SCALe " + MyFloatToStr(val) + "[V]\n";
	err = viPrintf(vi, str.c_str());
	if (err != Error_NoError)
		return err;
	myscaleU = val;
...
unsigned int __stdcall Drv_SaveWaveData(char*filename, char *fmt)
{
	AnsiString file_name = ExtractFilePath(filename);
	if (fmt == "8bmt")
	{
		str = ":SAVE:IMAGe[:STARt] " + file_name + "\n";
		err = viPrintf(vi, str.c_str());
		if (err != Error_NoError)
		return err;
		err = viPrintf(vi, ":SAVE:IMAGe:FORMat BMP8bit\n");
		if (err != Error_NoError)
		return err;
	}
	if (fmt == "24bmt")
	{
		...
		if (err != Error_NoError)
		return err;
		err = viPrintf(vi, ":SAVE:IMAGe:FORMat BMP24bit\n");
		if (err != Error_NoError)
		return err;
	}
	if (fmt == "24png")
	{
		...
		if (err != Error_NoError)
		return err;
		err = viPrintf(vi, ":SAVE:IMAGe:FORMat PNG\n");
		if (err != Error_NoError)
		return err;
	}
	if (fmt == "bin")
	{
		...
		if (err != Error_NoError)
		return err;
		err = viPrintf(vi, ":SAVE:WAVeform:FORMat BINary\n");
		if (err != Error_NoError)
		return err;
	}
	if (fmt == "mask")
	{
		str = ":SAVE:WAVeform[:STARt] " + file_name + "\n";
		err = viPrintf(vi, str.c_str());
		if (err != Error_NoError)
		return err;
		err = viPrintf(vi, ":SAVE:WAVeform:FORMat BINary\n");
